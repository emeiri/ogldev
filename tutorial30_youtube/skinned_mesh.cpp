...
bool SkinnedMesh::InitFromScene(const aiScene* pScene, const string& Filename)
{
    ...
    CountVerticesAndIndices(pScene, NumVertices, NumIndices); GCE

    ReserveSpace(NumVertices, NumIndices); GCE

    InitAllMeshes(pScene); GCE
...
int SkinnedMesh::GetBoneId(const aiBone* pBone)
{
    ...
    if (m_BoneNameToIndexMap.find(BoneName) == m_BoneNameToIndexMap.end()) {
        ...
        BoneIndex = (uint)m_BoneNameToIndexMap.size();
